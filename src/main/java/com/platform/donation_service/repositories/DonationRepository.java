...
public interface DonationRepository extends JpaRepository<DonationEntity, UUID> {
     ...
     * that match the provided campaign IDs and donation statuses.
     */
    ...
    /**
     * Finds all donations with a specific
     * status that were created before a given cutoff datetime.
     *
     * @param status The donation status to filter by.
     * @param cutoff The cutoff datetime; only
     *               donations created before this time will be returned.
     * @return A list of DonationEntity objects
